...
class DefaultPaymentFilterServiceTest extends TestCase
{
    ...
    public function testDifferentShippingAddress(): void
    {
        ...
        $result = $filterService->filterPaymentMethods($methodCollection, $filterContext);
        static::assertCount(4, $result->getElements(), 'no payment method should be removed, cause no configuration exists');
        ...
        $filterService = new DefaultPaymentFilterService($systemConfigService, PayoneSecuredInvoicePaymentHandler::class, null, null, null);
        $result = $filterService->filterPaymentMethods($methodCollection, $filterContext);
        static::assertCount(4, $result->getElements(), 'no payment method should be removed, cause a different shipping address is allowed');
        ...
        $filterService = new DefaultPaymentFilterService($systemConfigService, PayoneSecuredInvoicePaymentHandler::class, null, null, null);
        $result = $filterService->filterPaymentMethods($methodCollection, $filterContext);
        static::assertCount(3, $result->getElements(), 'payment method should be removed, cause a different shipping address is not allowed');
    }
