...
namespace PayonePayment\Storefront\Controller\AmazonPay;
...
class AmazonRedirectController extends StorefrontController
{
    ...
    #[Route(path: '/payone/redirect/amazon-pay/{uuid}', name: 'payment.payone_redirect.amazon', methods: ['GET'])]
    public function amazonRedirect(string $uuid): Response
    {
        $data = $this->connection->fetchOne('SELECT pay_data FROM payone_amazon_redirect WHERE id = ? LIMIT 1', [Uuid::fromHexToBytes($uuid)]);
